...
export const getAllInventories = asyncHandler(
  async (req: Request<{}, {}, {}, QueryType>, res: Response) => {
    ...
    if(req.query.filter) {
      ...
      count=await db.Inventory.count({where:{location:req.query.filter}});
    }
...
export const updateInventory=asyncHandler(async(req:Request<{id:string},{},BodyType>,res:Response,next:NextFunction)=>{
      const {name_en,name_ka,location,price}=req.body;
      const inventory=await db.Inventory.findOne({where:{uuid:req.params.id}});
      if(!inventory) {
        return next(new ErrorResponse(`no inventory found with id ${req.params.id}`,404))
      }
...
export const deleteInventory=asyncHandler(async(req:Request<{id:string},{},{},{limit:string,currentPage:string}>,res:Response,next:NextFunction)=>{
  const {currentPage,limit}=req.query;
  let lastPage;
  let newCurrentPage=+currentPage;
    const inventory=await db.Inventory.findOne({where:{uuid:req.params.id}});
    if(!inventory) {
      return next(new ErrorResponse(`No inventory is found with id of ${req.params.id}`,404));
    }
    await inventory.destroy();
    const count=await db.Inventory.count();
    if(count!==0 && count%+req.query.limit===0 && (+limit*(+currentPage-1))===count) {
